...
| 2026-09-14 | `—` | SYNC-20260914-002 | `init-detour/SKILL.md`, `reconcile-initiative/SKILL.md`, `validate-dor/SKILL.md`, `validate-dod/SKILL.md`, `generate-tap/SKILL.md` × camadas presentes, `.claude/CLAUDE.md` | Propaga o delta 2.13.0→2.14.0 do `tech-product-template` (TECH-574 + TECH-552): a taxonomia de trabalho passa a ser a do `AGENTS.md` do `tech-product-template`, §2 — detour é dois de três sinais e alterar o plano não é um deles —, a linha de delta no `Roadmap.md` vira pergunta de entrada em `init-detour` e conferência em `reconcile-initiative` (4a.1), a tabela de destinos da §1.7 **não entrou aqui** — o `AGENTS.md` deste repositório é documento próprio do projeto e não tem a seção correspondente —, e o jargão de projeto derivado sai dos exemplos de `validate-dor`, `validate-dod` e `generate-tap`. |
