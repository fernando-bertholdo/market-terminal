...
| 2026-09-12 | `PENDING` | SYNC-20260912-001 | `init-detour/SKILL.md` × 2 camadas, `.claude/CLAUDE.md` | Propagação do tech-product-template (TECH-475): `init-detour` ganha `--parent-issue` — o identificador da issue-pai no board passa a ser registrado no CONTEXT.md como **Issue-pai no board**, ligando board e filesystem sem que o diretório do detour saia de `.planning/detours/<nome>/`. Junto desce a regra "Classificar antes de criar issue" (origem TECH-459/TECH-471): apara primeiro, depois dois de três sinais — origem fora do plano, artefato próprio, paralelo à veia principal — fazem um detour, e o número de issues derivadas serve para reclassificar, nunca para decidir criar. O `AGENTS.md` da raiz **não** recebeu a seção: neste repositório ele é documentação de produto (Project Overview, Dev Commands, Architecture), não a camada de instrução pareada do template — não tem a âncora `### Regra de Ouro` nem a seção de skills onde a regra encaixa. |
