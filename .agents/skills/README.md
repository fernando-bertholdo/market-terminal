...
| 2026-09-03 | `PENDING` | `SYNC-20260903-004` | 7 `SKILL.md`, `prompts/tap-template.md`, `README.md` × 2 camadas | Propagação do tech-product-template: triagem TECH-106 PR-2/PR-3 das divergências entre camadas — frontmatter fora do topo, referência ao arquivo always-on, profundidade de link de exemplo, exigência de commit em pt-BR restaurada em organize-commits e pre-commit-check, e o `design-sprint` que faltava no índice de `.agents/`. O cabeçalho "Skills Essenciais (N)" passa a dizer a contagem real de cada camada. **Não** vieram nesta propagação: `SYNC-20260903-001` (saída do `TODO.md`) e `SYNC-20260903-002` (taxonomia sem o tipo `patch`), bloqueados pela adaptação local declarada na TECH-209 até a TECH-210 decidir; e `SYNC-20260903-003` (marcador de linhagem), que o passo 4 da `propagar-template` proíbe gravar enquanto a sincronização for parcial |
