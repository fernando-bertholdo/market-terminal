...
Você está configurando um novo projeto usando o template padrão.
...
- `Market Terminal` deve ser idêntico em README.md, AGENTS.md, Projeto.md, settings.json, etc.
- `web, sim, market, news, macro, auth, infra, scheduler, deploy, fetchers, docs, planning` deve ser idêntico em AGENTS.md, AGENTS.md, README.md
...
- Configurar `.agents/settings.json` com nome e descrição do projeto
- Configurar `.agents/config.toml` (mirror) se aplicável
...
- **Arquitetura Single Source of Truth** — AGENTS.md → Projeto.md → rules/*.md
- **Padrões de qualidade** — Code quality, testing, security
